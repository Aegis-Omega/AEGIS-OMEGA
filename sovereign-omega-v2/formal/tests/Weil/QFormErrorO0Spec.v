(*
  AEGIS Ω — preregistered O0 constructive-real quotient-stability recovery spec.

  TDD contract: this specification is committed before QFormErrorO0.v exists on
  this exact stack. It requires actual theorem proofs over the production O0
  carrier; it must not be satisfied by the exact-rational theorem alone.
*)
...
Proof.
  ...
  - apply CR_of_Q_le. apply Qle_bool_imp_le. reflexivity.
  - apply CR_of_Q_le. apply Qle_bool_imp_le. reflexivity.
  - apply CRltForget, CR_of_Q_pos. rewrite Qlt_alt. reflexivity.
  - apply CR_of_Q_le. apply Qle_bool_imp_le. reflexivity.
  - apply CR_of_Q_le. apply Qle_bool_imp_le. reflexivity.
