            ...
            "agents": {
                ...
                "automaton_2": {
                    ...
                    "constitutional_filter": {
                        ...
                        "require_signed_transition": True,
                    },
                },
            },
            "tools": {
...
def validate_manifest(manifest: dict[str, Any]) -> None:
    ...
    if manifest.get("provenance", {}).get("signature_mode") != SIGNATURE_MODE:
        raise ValueError("signature_mode is not the required GitHub OIDC mode")
    if not HASH_RE.fullmatch(manifest["provenance"]["parent_state_hash"]):
        raise ValueError("parent_state_hash is not a SHA-256 hex digest")
...
def output_paths(root: Path, output_dir: Path | None) -> dict[Path, str]:
    target = output_dir if output_dir is not None else root
    return {
        target / ".claude.json": "manifest",
        target / "skill-hashes.sha256": "hashes",
    }
...
def write_or_check(
    ...
    *,
    ...
    manifest, hash_lines = build_manifest(
        root,
        source_ref=source_ref,
        parent_state_hash=parent_state_hash,
    )
    ...
    outputs = output_paths(root, output_dir)
    ...
    for path, kind in outputs.items():
        ...
        path.write_text(rendered[kind], encoding="utf-8", newline="\n")
    print(
        f"Wrote cognitive manifest anchors for "
...
def main() -> int:
    ...
    parser.add_argument(
        "--ref",
        default=os.environ.get("AEGIS_MANIFEST_SOURCE_REF", "repository-state"),
    )
    parser.add_argument(
        "--parent-state-hash",
        default=os.environ.get("AEGIS_PARENT_STATE_HASH", ZERO_HASH),
    )
    parser.add_argument(
        "--root",
        default=None,
