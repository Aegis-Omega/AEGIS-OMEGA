...
| CLM-230 | VERIFIED | `aegis / automaton-3` binds the exact 129-test PR-1-through-PR-4 proofline, MCP and evidence-meta guards into candidate validation, deterministic repository-scoped receipts, replay packaging, checksums, and OIDC-attestation requests. | Verified | A | workflow, runner, validator, and adversarial suites |
| CLM-231 | VERIFIED | The exercised POSIX filesystem reference path rejects the enumerated handle/witness/root/FIFO/size/redirect/concurrent-rewrite falsifiers and implements verifier-gated evidence-only PR-3/PR-4 results; cross-process attestation, atomic snapshots, and effect-bound or production admission remain unestablished. | Verified | A | effect adapter/verifiers and exact security guards |
