    ...
    def process_transaction(self, request_payload: Any, raw_model_response: str) -> dict:
        ...
        try:
            ...
            witness_id = hashlib.sha256(
                f"{task_hash}{plan_hash}{output_hash}{index_hash}{self.model_version}".encode()
            ).hexdigest()
            ...
            try:
                conn.execute(
                    ...
                    INSERT INTO witnesses (
                        ...
                        index_hash, residual_delta, plan_hash, output_hash, parent_witness
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        witness_id,
                        ...
                        plan_hash,
                        output_hash,
                        request_payload.get("parent_witness")
                    )
                )
                conn.execute(
                    "INSERT INTO artifacts (witness_id, plan, output, tool_calls) VALUES (?, ?, ?, ?)",
                    (witness_id, proposal["plan"], proposal["output"], json.dumps({"tool_calls": proposal["tool_calls"], "signature": signature}))
                )
                conn.commit()
            finally:
                conn.close()
