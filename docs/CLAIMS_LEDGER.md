...
## QuantumDNA interpretation witness

The following records project the source-bound chain in
`docs/quantum-dna-witness-ledger.json`. Verified describes archived numerical
observations and repository witness integrity, not biological truth, an original
publication trajectory match, a new simulation run, or Control-Plane admission.
The source audit remains unchanged. Removed entries preserve the challenged
wording; their corrections are mandatory in the JSON ledger and witness chain.
The second mathematical document has not been supplied or audited.

| Claim ID | Ledger status | EQ | Claim under review |
|---|---|---|---|
| CLM-451 | Verified | EQ-A | The QuantumDNA witness ledger binds source bytes, complete claim semantics and ordered previous-entry hashes; verified integrity alone grants no runtime or biological authority. |
| CLM-452 | Verified | EQ-A | In the archived six-site one-hole GTG simulation, site-basis C_l1 at 25 fs is 0.21646991105146732 for gamma=0.1 eV and 0.1438771953946045 for gamma=0.5 eV; neither condition meets all preregistered classical-approximation criteria. |
| CLM-453 | Verified | EQ-A | The archived 37-state ELM reference runs record sampled ground-population 1-1/e crossings at 541.0821643286573 fs (shipped 1BNA parameters) and 775.5511022044088 fs (Hawke2010); these are distinct parameter replays, not an original-publication trajectory match. |
| CLM-454 | Removed | EQ-D | 1BNA published trajectory reproduced by matching Hawke scalar |
| CLM-455 | Removed | EQ-D | GTG C_l1 < 0.2 within 25 fs at gamma=0.1 eV |
| CLM-456 | Removed | EQ-D | gamma>=0.1 eV destroys coherence and establishes classical hopping |
| CLM-457 | Removed | EQ-D | The simulated environment is calibrated thermal noise |
| CLM-458 | Removed | EQ-D | GGG is a trapping state |
| CLM-459 | Removed | EQ-D | Central base barrier alone explains sequence ranking |
| CLM-460 | Removed | EQ-D | Complex Hermitian rho alone fixes real positive commutator margin |
| CLM-461 | Removed | EQ-D | Riemann-Lebesgue decay annihilates the spectrum |
| CLM-462 | Removed | EQ-D | Nonzero g_D squared establishes zero exclusion |
| CLM-463 | Removed | EQ-D | Every traceless Hermitian commutator observable -i[A,B], including the zero operator, has eigenvalues of both signs. |
| CLM-464 | Proposed | EQ-D | Single-parameter Hamiltonian ablations can distinguish coupling and detuning contributions to the GTG/GCG difference. |
| CLM-465 | Proposed | EQ-D | The simulated sequence/dephasing differences have a verified biological consequence in cells. |
| CLM-466 | Proposed | EQ-D | The complete second mathematical document can be audited for its commutator, Fourier-decay and zero-exclusion arguments. |

The implemented contract checks artifact identities, units, model dimensions,
actual collapse operators, and declared Hamiltonian changes before a future run.
Passing preflight grants no authority and does not establish causal isolation of
all baseline/candidate parameters. See `genomics/quantum_dna/README.md` for the
contract, commands, evidence boundary and correction details.
