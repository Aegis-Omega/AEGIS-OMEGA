...
//! Tracks ack timeout e3 rate per gossip broadcast epoch.
//! HIGH_ACK_TIMEOUT_E3_THRESHOLD = 8: rate_pct > 8 → high_ack_timeout_e3

use sha2::{Sha256, Digest};

pub const ACK_TIMEOUT_E3_GENESIS_HASH: [u8; 32] = [0u8; 32];
pub const HIGH_ACK_TIMEOUT_E3_THRESHOLD: u32 = 8;

#[derive(Debug, Clone, PartialEq)]
pub struct GossipAckTimeoutE3Entry {
    pub epoch_end:              u64,
    pub unacknowledged_msgs:    u32,
    pub total_sent:             u32,
    pub unacknowledged_rate_pct: u32,
    pub high_ack_timeout_e3:    bool,
...
fn compute_hash(
    ...
    h.update([high_ack_timeout_e3 as u8]);
    h.finalize().into()
}

pub struct GossipAckTimeoutE3Log {
    pub entries: Vec<GossipAckTimeoutE3Entry>,
}

impl GossipAckTimeoutE3Log {
    pub fn new() -> Self {
    ...
    pub fn record(
        ...
        let high_ack_timeout_e3 = unacknowledged_rate_pct > HIGH_ACK_TIMEOUT_E3_THRESHOLD;
        let prev = self
            .entries
            .last()
            .map(|e| e.entry_hash)
            .unwrap_or(ACK_TIMEOUT_E3_GENESIS_HASH);
        ...
        self.entries.push(GossipAckTimeoutE3Entry {
            epoch_end,
            unacknowledged_msgs,
            total_sent,
            unacknowledged_rate_pct,
    ...
    pub fn high_ack_timeout_e3_count(&self) -> usize {
        self.entries.iter().filter(|e| e.high_ack_timeout_e3).count()
    }

    pub fn total_unacknowledged_msgs(&self) -> u64 {
        self.entries
            .iter()
            .map(|e| e.unacknowledged_msgs as u64)
    ...
    pub fn mean_rate_pct(&self) -> u32 {
        ...
        let sum: u64 = self
            .entries
            .iter()
            .map(|e| e.unacknowledged_rate_pct as u64)
            .fold(0u64, |acc, v| acc.saturating_add(v));
        (sum / self.entries.len() as u64) as u32
    }

    pub fn verify_chain(&self) -> (bool, Option<usize>) {
        let mut prev = ACK_TIMEOUT_E3_GENESIS_HASH;
...
impl Default for GossipAckTimeoutE3Log {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_record_fields_correct_flag_true() {
        let mut log = GossipAckTimeoutE3Log::new();
    ...
    fn test_threshold_constant_value() {
        assert_eq!(HIGH_ACK_TIMEOUT_E3_THRESHOLD, 8u32);
    }
    ...
    fn test_first_prev_hash_is_genesis() {
        ...
        assert_eq!(e.prev_hash, ACK_TIMEOUT_E3_GENESIS_HASH);
    }
    ...
    fn test_verify_chain_empty() {
        let log = GossipAckTimeoutE3Log::new();
    ...
    fn test_determinism_same_inputs_same_hash() {
        let mut log1 = GossipAckTimeoutE3Log::new();
        let mut log2 = GossipAckTimeoutE3Log::new();
        let mut log3 = GossipAckTimeoutE3Log::new();
    ...
    #[test]
    fn test_high_ack_timeout_e3_count_mixed() {
        let mut log = GossipAckTimeoutE3Log::new();
        // rate = 0, flag false
        log.record(19000, 0, 100);
        // rate = 8, flag false (boundary)
        log.record(20000, 8, 100);
        // rate = 9, flag true
        log.record(21000, 9, 100);
        // rate = 50, flag true
        log.record(22000, 50, 100);
        assert_eq!(log.high_ack_timeout_e3_count(), 2);
    }
    ...
    fn test_default_zero_entries() {
        let log = GossipAckTimeoutE3Log::default();
