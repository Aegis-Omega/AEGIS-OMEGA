...
ROOT="$(pwd)"
ASSET_DIR="$ROOT/aegisomega-webgpu/conservative-consensus-crdt-v2"
WORK="/tmp/aegis-ccrdt"
...
NFEG_FILE="NoFreeEpistemicGainV1.lean"
MACC_FILE="MultiAgentConservativeConsensusV1.lean"
CRDT_FILE="ConservativeConsensusCRDTV1.lean"
NFEG_SHA256="$(sha256sum "$SRC/$NFEG_FILE" | awk '{print $1}')"
MACC_SHA256="$(sha256sum "$SRC/$MACC_FILE" | awk '{print $1}')"
CRDT_SHA256="$(sha256sum "$SRC/$CRDT_FILE" | awk '{print $1}')"
PYTHON_STATUS="NOT_RUN"

write_status() {
  python3 - "$STATUS" "$HEAD_SHA" "$NFEG_SHA256" "$MACC_SHA256" "$CRDT_SHA256" "$PYTHON_STATUS" "$1" "$2" "$3" "$4" <<'PY'
import json,sys
path,head,nfeg_sha,macc_sha,crdt_sha,python_status,stage,ok,leanv,detail=sys.argv[1:]
payload={
  "receipt_kind":"AEGIS_CONSERVATIVE_CONSENSUS_CRDT_REPLAY_V2",
  "transport_generation":"STABLE_MACC_PARENT_V2",
  "head_sha":head,
  "nfeg_source_sha256":nfeg_sha,
  "macc_source_sha256":macc_sha,
  "crdt_source_sha256":crdt_sha,
  "python_regressions":python_status,
  "lean_target":"4.33.1",
  "mathlib_sha":"0df444a360eaa60ab8c11dca51a86af692955474",
  "target_modules":[
    "NoFreeEpistemicGainV1",
    "MultiAgentConservativeConsensusV1",
    "ConservativeConsensusCRDTV1"
  ],
  "stage":stage,
  "verified":ok=="true",
...
write_status "START" "false" "" "conservative CRDT replay starting"
echo "HEAD_SHA=$HEAD_SHA"
echo "NFEG_SOURCE_SHA256=$NFEG_SHA256"
echo "MACC_SOURCE_SHA256=$MACC_SHA256"
echo "CRDT_SOURCE_SHA256=$CRDT_SHA256"

echo "=== PYTHON CRDT REGRESSIONS ==="
if ! PYTHONPATH="$ROOT/sovereign-omega-v2/python"   python3 "$ROOT/sovereign-omega-v2/python/tests/test_conservative_consensus_crdt.py" -v   2>&1 | tee "$PYLOG"; then
...
cp "$SRC/$NFEG_FILE" "$WORK/mathlib/$NFEG_FILE"
cp "$SRC/$MACC_FILE" "$WORK/mathlib/$MACC_FILE"
cp "$SRC/$CRDT_FILE" "$WORK/mathlib/$CRDT_FILE"

NFEG_COPY="$(sha256sum "$WORK/mathlib/$NFEG_FILE" | awk '{print $1}')"
MACC_COPY="$(sha256sum "$WORK/mathlib/$MACC_FILE" | awk '{print $1}')"
CRDT_COPY="$(sha256sum "$WORK/mathlib/$CRDT_FILE" | awk '{print $1}')"

test "$NFEG_COPY" = "$NFEG_SHA256"
test "$MACC_COPY" = "$MACC_SHA256"
test "$CRDT_COPY" = "$CRDT_SHA256"
...
if ! lean -o "$OUT/NoFreeEpistemicGainV1.olean"   "$WORK/mathlib/NoFreeEpistemicGainV1.lean"; then
  write_status "NFEG_LEAN_COMPILE_FAILED" "false" "$LEANV" "see replay.log"
  exit 0
fi

echo "=== COMPILE MultiAgentConservativeConsensusV1 ==="
if ! lean -o "$OUT/MultiAgentConservativeConsensusV1.olean"   "$WORK/mathlib/MultiAgentConservativeConsensusV1.lean"; then
  write_status "MACC_LEAN_COMPILE_FAILED" "false" "$LEANV" "see replay.log"
  exit 0
fi

echo "=== COMPILE ConservativeConsensusCRDTV1 ==="
if ! lean -o "$OUT/ConservativeConsensusCRDTV1.olean"   "$WORK/mathlib/ConservativeConsensusCRDTV1.lean"; then
  write_status "CRDT_LEAN_COMPILE_FAILED" "false" "$LEANV" "see replay.log"
  exit 0
fi

cat > "$WORK/mathlib/CRDTAudit.lean" <<'LEAN'
...
echo "=== AXIOM AUDIT ==="
if ! lean "$WORK/mathlib/CRDTAudit.lean" 2>&1 | tee "$AXLOG"; then
...
write_status "VERIFIED_EXACT_SOURCE" "true" "$LEANV"   "CRDT Python suite passed; exact NFEG, MACC and CRDT Lean modules compiled; axiom audit has no sorryAx"

echo "AEGIS_CONSERVATIVE_CONSENSUS_CRDT_REPLAY_V2=PASS"
exit 0
