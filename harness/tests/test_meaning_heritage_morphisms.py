...
def test_heritage_semantic_proof_must_be_authenticated_and_input_bound():
    ...
    env = SemanticLineageEnvelopeV1(
        ...
        preservation_edges=(PreservationEdge(a.claim_digest, b.claim_digest, PreservationRelation.SEMANTIC_EQUIVALENCE, fake_root),),
...
def test_predecessors_require_trusted_store():
    ...
    env = SemanticLineageEnvelopeV1(
        ...
        preservation_edges=(PreservationEdge(a.claim_digest, a.claim_digest, PreservationRelation.SAME_CLAIM_ROOT, proof.root),),
        declared_omission_digests=(),
        declared_additions=(),
        uncertainty_bps=0,
    )
    verifier = HeritageVerifierV13(
        verifier_root=h("HV", 1), policy_root=h("HP", 1), proof_store=proof_store
    )
...
def make_registry(proof_store: ProofStore, heritage_store: dict[str, HeritageReceiptV1]):
    common = dict(verifier_root=h("VERIFIER", 1), policy_root=h("POLICY", 1), proof_store=proof_store)
...
def test_carrier_requires_authenticated_subject_bound_proof():
    ...
    source = TheoremContextV1(h("T", 1), h("C", 1), h("H", 1), h("N", 1), h("G", 1))
    target = TheoremContextV1(h("T", 2), h("C", 2), h("H", 2), h("N", 2), h("G", 2))
    transport_map = h("MAP", 1)
    subject = h("AEGIS_CARRIER_TRANSPORT_SUBJECT_V1", {
        "source": asdict(source), "target": asdict(target), "transport_map_root": transport_map
    })
    proof = ProofArtifactReceiptV1(subject, h("KERNEL", 1), h("KPOL", 1), "PASS", h("CTX", 1))
    proof_root = store.put(proof)
    env = MorphismEnvelopeV1(
        "carrier-1", MorphismKind.CARRIER, source.carrier_root, target.carrier_root,
...
def test_composition_requires_trusted_predecessors_and_endpoint_match():
    ...
    # Use SPACE twice because each proof subject is easy to bind explicitly.
    a, b, c = h("D", "A"), h("D", "B"), h("D", "C")

    def make_space(mid: str, src: str, dst: str):
        subject_root = h("SUBJECT", mid)
        proof_subject = h("AEGIS_SPACE_SUBJECT_V1", {
            "subject_root": subject_root,
            "source_space_root": src,
            "target_space_root": dst,
        })
        image = ProofArtifactReceiptV1(proof_subject, h("K", mid + "i"), h("P", mid + "i"), "PASS", h("CTX", mid + "i"))
        adm = ProofArtifactReceiptV1(proof_subject, h("K", mid + "a"), h("P", mid + "a"), "PASS", h("CTX", mid + "a"))
        store.put(image); store.put(adm)
        return MorphismEnvelopeV1(
            f"space-{mid}", MorphismKind.SPACE, src, dst,
    ...
    assert r1 and r2
    morph_store.put(r1); morph_store.put(r2)

    composed = make_space("comp", a, c)
    result, receipt = registry.compose_and_issue(
        r1, r2, composed,
        receipt_store=morph_store,
        composition_verifier_root=h("COMPVER", 1),
        composition_policy_root=h("COMPPOL", 1),
    )
    assert result.status == "PASS"
    assert receipt is not None
