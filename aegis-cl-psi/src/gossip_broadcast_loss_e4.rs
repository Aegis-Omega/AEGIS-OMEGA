...
//! Tracks loss e4 rate per gossip broadcast epoch.
//! HIGH_LOSS_E4_THRESHOLD = 3: rate_pct > 3 → high_loss_e4

use sha2::{Sha256, Digest};

pub const LOSS_E4_GENESIS_HASH: [u8; 32] = [0u8; 32];
pub const HIGH_LOSS_E4_THRESHOLD: u32 = 3;

#[derive(Debug, Clone, PartialEq)]
pub struct GossipLossE4Entry {
    pub epoch_end:     u64,
    pub lost_msgs:     u32,
    pub total_sent:    u32,
    pub lost_rate_pct: u32,
    pub high_loss_e4:  bool,
...
fn compute_hash(
    ...
    h.update([high_loss_e4 as u8]);
    h.finalize().into()
}

pub struct GossipLossE4Log {
    pub entries: Vec<GossipLossE4Entry>,
}

impl GossipLossE4Log {
    pub fn new() -> Self {
        Self { entries: Vec::new() }
    }

    pub fn record(&mut self, epoch_end: u64, lost_msgs: u32, total_sent: u32) -> &GossipLossE4Entry {
        let denom = total_sent.max(1) as u64;
        let lost_rate_pct = ((lost_msgs as u64).saturating_mul(100) / denom).min(100) as u32;
        let high_loss_e4 = lost_rate_pct > HIGH_LOSS_E4_THRESHOLD;
        let prev = self.entries.last().map(|e| e.entry_hash).unwrap_or(LOSS_E4_GENESIS_HASH);
        let entry_hash = compute_hash(&prev, epoch_end, lost_msgs, total_sent, lost_rate_pct, high_loss_e4);
        self.entries.push(GossipLossE4Entry {
            epoch_end,
            lost_msgs,
            total_sent,
            lost_rate_pct,
    ...
    pub fn high_loss_e4_count(&self) -> usize {
        self.entries.iter().filter(|e| e.high_loss_e4).count()
    }
    ...
    pub fn verify_chain(&self) -> (bool, Option<usize>) {
        let mut prev = LOSS_E4_GENESIS_HASH;
        for (i, e) in self.entries.iter().enumerate() {
            if e.prev_hash != prev {
                return (false, Some(i));
            }
            let expected = compute_hash(&prev, e.epoch_end, e.lost_msgs, e.total_sent, e.lost_rate_pct, e.high_loss_e4);
...
impl Default for GossipLossE4Log {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn record_fields_correct_flag_true() {
        let mut log = GossipLossE4Log::new();
        let e = log.record(1000, 10, 100);
        assert_eq!(e.epoch_end, 1000);
        assert_eq!(e.lost_msgs, 10);
        assert_eq!(e.total_sent, 100);
        assert_eq!(e.lost_rate_pct, 10);
        assert!(e.high_loss_e4);
    }

    #[test]
    fn flag_false_at_threshold_boundary() {
        let mut log = GossipLossE4Log::new();
        // 3 out of 100 = 3%, which is exactly threshold, not > threshold
    ...
    fn threshold_constant_value_is_3() {
        assert_eq!(HIGH_LOSS_E4_THRESHOLD, 3);
    }
    ...
    fn first_prev_hash_is_genesis() {
        ...
        assert_eq!(e.prev_hash, LOSS_E4_GENESIS_HASH);
    }
    ...
    fn determinism_same_inputs_same_hash() {
        let mut log1 = GossipLossE4Log::new();
        let mut log2 = GossipLossE4Log::new();
        let mut log3 = GossipLossE4Log::new();
    ...
    #[test]
    fn high_loss_e4_count_mixed_log() {
        let mut log = GossipLossE4Log::new();
        log.record(19000, 1, 100); // 1% - not high
        log.record(20000, 5, 100); // 5% - high
        log.record(21000, 3, 100); // 3% - not high (boundary)
        log.record(22000, 4, 100); // 4% - high
        assert_eq!(log.high_loss_e4_count(), 2);
    }
    ...
    fn total_lost_msgs_sums_correctly() {
        ...
        assert_eq!(log.total_lost_msgs(), 60);
    }
    ...
    fn default_has_zero_entries() {
        let log = GossipLossE4Log::default();
