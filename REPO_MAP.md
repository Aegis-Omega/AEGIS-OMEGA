...
| `vertex/serve.py` | FastAPI "constitutional-proxy" / `aegis-platform`; owns bounded `/agents/dispatch`, verifies request-bound GitHub OIDC, and bundles the authority runtime plus constitutional workspace anchors | Separate manual deploy via `vertex/cloudbuild.yaml`; not the `aegis-vertex` bridge; candidate OIDC revision is not production-established |
