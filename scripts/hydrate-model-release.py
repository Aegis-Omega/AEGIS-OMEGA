#!/usr/bin/env python3
"""Hydrate model weights from an AEGIS repository-owned release mirror.

This tool never treats successful download as execution authority. It only
reconstructs and hash-verifies model bytes into `models/weights/<package>`.
"""
...
def main() -> int:
    ...
    parser.add_argument("--repo", help="owner/name; required unless --asset-dir is used")
    ...
    if mirror["state"] != "MIRRORED_VERIFIED":
        raise SystemExit(
            f"package {args.package_id} is not an admitted repo mirror; state={mirror['state']}"
        )
    if package["weight_availability"] != "OPEN_WEIGHTS":
        raise SystemExit("remote-only model packages cannot be hydrated")
    ...
    if args.asset_dir:
        ...
        if not args.repo:
            raise SystemExit("--repo is required unless --asset-dir is supplied")
    ...
    try:
        ...
        if not release_manifest_path.is_file() or not manifest_digest_path.is_file():
            raise SystemExit("release is missing manifest or manifest digest")

        digest_line = manifest_digest_path.read_text(encoding="utf-8").strip().split()
        if len(digest_line) < 1:
            raise SystemExit("invalid release manifest digest file")
        ...
        if release_manifest.get("package_id") != args.package_id:
            raise SystemExit("release manifest package_id mismatch")
        if release_manifest.get("authority") != "EVIDENCE_ONLY":
            raise SystemExit("release manifest authority boundary changed")
        if release_manifest.get("source") != package.get("source"):
            raise SystemExit("release source does not match canonical package source")
            ...
            with destination.open("wb") as output:
                for part in file_record["parts"]:
                    ...
                    if not asset.is_file():
                        raise SystemExit(f"missing release asset: {part['asset']}")
        ...
        receipt = {
            ...
            "release_tag": mirror["release_tag"],
