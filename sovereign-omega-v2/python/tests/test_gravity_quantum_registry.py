...
from gravity_quantum_registry import (
    RegistryError,
    load_registry,
    validate_registry,
)
...
class GravityQuantumRegistryTests(unittest.TestCase):
    def test_current_registry_passes(self):
        ...
        self.assertIn(
            "AZIZ_HOWL_CLASSICAL_ENTANGLEMENT_2025",
            result["contested_groups"],
        )
    ...
    def test_contested_model_without_counteranalyses_denies(self):
        ...
        bad["models"] = [
            model
            for model in bad["models"]
            if model.get("evidence_class") != "PREPRINT_CRITIQUE"
        ]
    ...
    def test_registry_cannot_self_establish_disputed_physics(self):
        ...
        target = next(
            model
            for model in bad["models"]
            if model["id"] == "AZIZ_HOWL_2025_EQ10_SMALL_DX"
        )
