...
| CLM-231 | UNRESOLVED | Two named models differ measurably in capability on the six-task MCB-001 battery. | Preregistered, not run | D | `sovereign-omega-v2/scripts/model-capability/PREREGISTRATION.md`; graders validated in `test/unit/model-capability-graders.test.ts`; **no `results/` file exists — the battery has never been executed** (`ANTHROPIC_API_KEY` unset in the authoring sandbox) |
| CLM-232 | NOT_ESTABLISHED | A capability difference between two models bears on which model authored a given commit. | Rejected a priori | D | Preregistered non-inference, `PREREGISTRATION.md` §2. Authorship is a provenance question (CLM-206); MCB-001 collects no provenance evidence and cannot move it in either direction |
