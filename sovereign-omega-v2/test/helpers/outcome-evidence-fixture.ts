...
export const H = (character: string) => character.repeat(64) as SHA256Hex
export const SEQ = (value: number) => BigInt(value) as SequenceNumber

export interface OutcomeClosureFixture {
  ...
  readonly trustPolicy: OutcomeVerifierTrustPolicyV1
  readonly input: AdaptationOutcomeInput
}

export async function outcomeSelfModel(
...
export async function certifyOutcomeInput(
  input: AdaptationOutcomeInput,
  verifier: OutcomeVerifierIdentityV1,
  verifierKeypair: Awaited<ReturnType<typeof generateKeypair>>,
): Promise<AdaptationOutcomeInput> {
  const { evidence_certificate: _certificate, ...unsignedInput } = input
...
/** Construct adversarial externally signed input without the production normalizer. */
export async function certifyUncheckedOutcomeInputForTest(
  input: AdaptationOutcomeInput,
  verifier: OutcomeVerifierIdentityV1,
  verifierKeypair: Awaited<ReturnType<typeof generateKeypair>>,
): Promise<AdaptationOutcomeInput> {
  const { evidence_certificate: _certificate, ...unsignedInput } = input
  const evidenceBundleDigest = await hashValue({
    domain: 'AEGIS_OUTCOME_EVIDENCE_BUNDLE_V1',
    evidence: {
      ...unsignedInput,
      terminal_execution: unsignedInput.terminal_execution ?? null,
    },
  })
  const unsignedCertificate: Omit<OutcomeEvidenceCertificateV1, 'signature'> = {
    certificate_kind: 'AEGIS_OUTCOME_EVIDENCE_CERTIFICATE_V1',
...
export async function createOutcomeClosureFixture(): Promise<OutcomeClosureFixture> {
  ...
  const baseline = await outcomeSelfModel(verifierTrustRoot)
  ...
  const post = await outcomeSelfModel(verifierTrustRoot, { capability_root: H('9') })
  ...
  return {
    ...
    verifier,
    trustPolicy,
    input: await certifyOutcomeInput(unsignedInput, verifier, verifierKeypair),
  }
}

export function outcomeReplayEvidence(
  fixture: OutcomeClosureFixture,
  input: AdaptationOutcomeInput = fixture.input,
) {
  return { input, trust_policy: fixture.trustPolicy }
}

export function trustedOutcomeReplayContext(fixture: OutcomeClosureFixture) {
  return {
    expected_governed_policy_root: H('3'),
    expected_operator_public_key: fixture.operatorPublicKey,
  }
}

export async function createOutcomeEvidenceArtifactFixture(): Promise<OutcomeEvidenceArtifactV1> {
  const fixture = await createOutcomeClosureFixture()
  const observed = await recordOutcomeAssessment(
    MetacognitiveLoop.empty(),
    fixture.input,
    await verifyOutcomeVerifierTrustPolicyV1(
      fixture.trustPolicy,
      H('3'),
