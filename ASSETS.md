...
| `agents/` department engine | **LIVE (registered 2026-07-03)** | All 39 departments created as Anthropic Managed Agents (`agent_registry.json` holds the IDs); `cognitive_pipeline.py`, `coordinator.py`, `evolution.py`, `red_team.py`, `revenue_engine.py`, `tool_runner.py`, `adaptive_lineage.json`. Fixed: `agents.yaml` carried a literal `${AEGIS_DEFAULT_MODEL:-…}` shell placeholder as the model ID |
