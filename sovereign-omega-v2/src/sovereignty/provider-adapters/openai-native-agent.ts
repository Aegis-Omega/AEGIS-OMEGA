...
// ============================================================
//
// The concrete @openai/agents runner is intentionally injected. This keeps
// credentials and network execution outside the deterministic contract and
// makes selection/receipt logic replay-testable without live API calls.
...
export interface OpenAIAgentRunnerInputV1 {
  readonly agent_name: 'AEGIS OpenAI Provider Agent'
  readonly instructions: string
  readonly task: string
}

export interface OpenAIAgentRunnerOutputV1 {
  readonly final_output: string
  readonly run_id: string
  readonly model: string
}

export interface OpenAIAgentRunnerV1 {
  run(input: OpenAIAgentRunnerInputV1): Promise<OpenAIAgentRunnerOutputV1>
}

export interface OpenAINativeAgentExecutionReceiptV1 {
  readonly schema_version: typeof OPENAI_NATIVE_AGENT_ADAPTER_SCHEMA_VERSION
  readonly provider_id: 'openai'
  readonly agent_id: 'provider-agent:openai'
  readonly provider_selection_receipt_root: SHA256Hex
  readonly native_run_id: string
  readonly model: string
...
export interface OpenAINativeAgentExecuteInputV1 {
...
export interface OpenAINativeAgentExecuteResultV1 {
  readonly selection: ProviderNativeAgentSelectionReceiptV1
  readonly output: OpenAIAgentRunnerOutputV1 | null
  readonly receipt: OpenAINativeAgentExecutionReceiptV1 | null
}

const OPENAI_AGENT_INSTRUCTIONS = [
  'You are the OpenAI provider-native agent inside AEGIS Omega.',
  'Operate only within the task supplied by the AEGIS conductor.',
  'Do not claim or exercise repository write, merge, deploy, financial, or other external mutation authority.',
  'Return evidence-oriented results to the conductor; authority remains external to this agent.',
].join(' ')

export function createOpenAINativeAgentAdapterV1(
  options: { readonly runner: OpenAIAgentRunnerV1 },
): {
  execute(input: OpenAINativeAgentExecuteInputV1): Promise<OpenAINativeAgentExecuteResultV1>
} {
  return {
    async execute(input) {
      if (!input.task.trim()) throw new TypeError('task must be non-empty')

      const selection = await selectProviderNativeAgentV1(
        input.snapshot,
        input.registry,
        {
          required_capabilities: ['AGENT_EXECUTION', 'MODEL_INFERENCE'],
          allowed_providers: ['openai'],
          preferred_provider_order: ['openai'],
          current_generation: input.current_generation,
          max_observation_age_generations: input.max_observation_age_generations,
        },
      )

      if (selection.outcome !== 'SELECTED' || selection.provider_id !== 'openai') {
        return { selection, output: null, receipt: null }
      }

      const task_hash = await sha256Hex(canonicalizeJCS({
        domain: 'AEGIS_OPENAI_NATIVE_AGENT_TASK_V1',
        task: input.task,
      })) as SHA256Hex
      ...
      if (!output.run_id.trim()) throw new TypeError('native OpenAI run_id must be non-empty')
      if (!output.model.trim()) throw new TypeError('native OpenAI model must be non-empty')

      const output_hash = await sha256Hex(canonicalizeJCS({
        domain: 'AEGIS_OPENAI_NATIVE_AGENT_OUTPUT_V1',
        native_run_id: output.run_id,
        model: output.model,
        final_output: output.final_output,
      })) as SHA256Hex

      const body = {
        schema_version: OPENAI_NATIVE_AGENT_ADAPTER_SCHEMA_VERSION,
        provider_id: 'openai' as const,
        agent_id: 'provider-agent:openai' as const,
      ...
      const receipt_root = await sha256Hex(canonicalizeJCS({
        domain: 'AEGIS_OPENAI_NATIVE_AGENT_EXECUTION_RECEIPT_V1',
        receipt: body,
      })) as SHA256Hex
