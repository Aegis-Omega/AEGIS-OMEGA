...
//! Tracks rebroadcast e3 rate per gossip broadcast epoch.
//! HIGH_REBROADCAST_E3_THRESHOLD = 12: rate_pct > 12 → high_rebroadcast_e3

use sha2::{Sha256, Digest};

pub const REBROADCAST_E3_GENESIS_HASH: [u8; 32] = [0u8; 32];
pub const HIGH_REBROADCAST_E3_THRESHOLD: u32 = 12;

#[derive(Debug, Clone, PartialEq)]
pub struct GossipRebroadcastE3Entry {
    pub epoch_end:            u64,
    pub rebroadcast_count:    u32,
    pub total_sent:           u32,
    pub rebroadcast_rate_pct: u32,
    pub high_rebroadcast_e3:  bool,
...
fn compute_hash(
    ...
    h.update([high_rebroadcast_e3 as u8]);
    h.finalize().into()
}

pub struct GossipRebroadcastE3Log {
    pub entries: Vec<GossipRebroadcastE3Entry>,
}

impl GossipRebroadcastE3Log {
    pub fn new() -> Self {
    ...
    pub fn record(
        ...
        let high_rebroadcast_e3 = rebroadcast_rate_pct > HIGH_REBROADCAST_E3_THRESHOLD;
        let prev = self
            .entries
            .last()
            .map(|e| e.entry_hash)
            .unwrap_or(REBROADCAST_E3_GENESIS_HASH);
        ...
        self.entries.push(GossipRebroadcastE3Entry {
            epoch_end,
            rebroadcast_count,
            total_sent,
            rebroadcast_rate_pct,
    ...
    pub fn high_rebroadcast_e3_count(&self) -> usize {
        self.entries.iter().filter(|e| e.high_rebroadcast_e3).count()
    }
    ...
    pub fn verify_chain(&self) -> (bool, Option<usize>) {
        let mut prev = REBROADCAST_E3_GENESIS_HASH;
...
impl Default for GossipRebroadcastE3Log {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn record_fields_correct_flag_true_when_above_threshold() {
        let mut log = GossipRebroadcastE3Log::new();
        // rebroadcast_count=20, total_sent=100 => rate=20 > 12 => high=true
    ...
    fn rate_pct_capped_at_100() {
        let mut log = GossipRebroadcastE3Log::new();
        // rebroadcast_count=200, total_sent=100 => raw=200 capped to 100
    ...
    fn total_sent_zero_no_div_by_zero() {
        let mut log = GossipRebroadcastE3Log::new();
        // total_sent=0 => denom=1 => rate = rebroadcast_count*100
    ...
    fn threshold_constant_value_is_12() {
        assert_eq!(HIGH_REBROADCAST_E3_THRESHOLD, 12);
    }
    ...
    fn first_prev_hash_equals_genesis() {
        ...
        assert_eq!(e.prev_hash, REBROADCAST_E3_GENESIS_HASH);
    }
    ...
    fn verify_chain_empty_returns_true_none() {
        let log = GossipRebroadcastE3Log::new();
    ...
    fn determinism_same_inputs_same_hash() {
        let mut log1 = GossipRebroadcastE3Log::new();
        let mut log2 = GossipRebroadcastE3Log::new();
        let mut log3 = GossipRebroadcastE3Log::new();
        log1.record(17000, 10, 80);
        log2.record(17000, 10, 80);
        log3.record(17000, 10, 80);
    ...
    #[test]
    fn high_rebroadcast_e3_count_mixed_log() {
        let mut log = GossipRebroadcastE3Log::new();
        // rate=5 => not high
        log.record(18000, 5, 100);
        // rate=20 => high
        log.record(19000, 20, 100);
        // rate=12 => not high (equal, not greater)
        log.record(20000, 12, 100);
        // rate=13 => high
        log.record(21000, 13, 100);
        assert_eq!(log.high_rebroadcast_e3_count(), 2);
    }
    ...
    fn default_produces_zero_entries() {
        let log = GossipRebroadcastE3Log::default();
