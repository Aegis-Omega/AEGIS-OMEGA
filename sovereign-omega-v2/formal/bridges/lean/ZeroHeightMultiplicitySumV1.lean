...
/-!
AEGIS Ω — finite multiplicity-weighted height sum for nontrivial zeta zeros v1.

This lane does only three things:
1. constructs the finite carrier of nontrivial zeta zeros with `|Im(rho)| ≤ T`;
2. records the natural analytic multiplicity of every member and proves it positive;
3. defines the corresponding multiplicity-weighted finite Mellin sum using `Finset.sum`.

No infinite zero sum, limit, explicit formula, critical-line theorem, or RH claim is introduced.

FINITE_HEIGHT_MULTIPLICITY_SUM_ONLY
...
/-- The height-bounded nontrivial-zero carrier is finite. -/
theorem nontrivial_zero_height_set_finite_v1 (T : ℝ) :
  ...
  have hre_nonneg : 0 ≤ rho.re := hstrip.1.le
  have hre_le : rho.re ≤ 1 := hstrip.2.le
  constructor
  · simp only [Metric.mem_closedBall, dist_zero_right]
    calc
      ‖rho‖ ≤ |rho.re| + |rho.im| := Complex.norm_le_abs_re_add_abs_im rho
      _ = rho.re + |rho.im| := by rw [abs_of_nonneg hre_nonneg]
      _ ≤ 1 + T := add_le_add hre_le him
      _ = T + 1 := by ring
  · exact mem_riemannZetaZeros.mpr hz

private theorem riemannZeta_order_ne_top_height_sum_local_v1
    {rho : ℂ} (hrho1 : rho ≠ 1) :
    analyticOrderAt riemannZeta rho ≠ ⊤ := by
  intro htop
  have hlocal : riemannZeta =ᶠ[𝓝 rho] (fun _ : ℂ => 0) := by
    filter_upwards [analyticOrderAt_eq_top.mp htop] with z hz
    exact hz
  have hrhoU : rho ∈ ({1}ᶜ : Set ℂ) := by
    simpa using hrho1
...
/-- Natural analytic multiplicity used on the finite height carrier. -/
def RiemannZeroMultiplicityAtHeightV1 (rho : ℂ) : ℕ :=
  analyticOrderNatAt riemannZeta rho

/-- Every member of the finite height carrier has strictly positive multiplicity. -/
theorem zero_height_multiplicity_pos_v1
    {T : ℝ} {rho : ℂ} (hrho : rho ∈ NontrivialZeroHeightSetV1 T) :
    0 < RiemannZeroMultiplicityAtHeightV1 rho := by
  change riemannZeta rho = 0 ∧
      (¬ ∃ n : ℕ, rho = -(2 : ℂ) * (n + 1)) ∧ |rho.im| ≤ T at hrho
  rcases hrho with ⟨hz, hnontrivial, _⟩
  have hstrip := nontrivial_zero_critical_strip_height_sum_local_v1 hz hnontrivial
  have hrho1 : rho ≠ 1 := by
    intro h
    subst rho
  ...
  have hAn : AnalyticAt ℂ riemannZeta rho :=
    analyticOn_riemannZeta rho (by simpa using hrho1)
  have horder_ne0 : analyticOrderAt riemannZeta rho ≠ 0 :=
    hAn.analyticOrderAt_ne_zero.mpr hz
  have horder_netop : analyticOrderAt riemannZeta rho ≠ ⊤ :=
    riemannZeta_order_ne_top_height_sum_local_v1 hrho1
  apply Nat.pos_of_ne_zero
  intro hnat
  have hnat' : analyticOrderNatAt riemannZeta rho = 0 := by
    simpa [RiemannZeroMultiplicityAtHeightV1] using hnat
  apply horder_ne0
  calc
    analyticOrderAt riemannZeta rho =
        (analyticOrderNatAt riemannZeta rho : ℕ∞) :=
      (Nat.cast_analyticOrderNatAt horder_netop).symm
    _ = 0 := by simp [hnat']

/-- Canonical finite carrier corresponding exactly to `NontrivialZeroHeightSetV1 T`. -/
def NontrivialZeroHeightFinsetV1 (T : ℝ) : Finset ℂ :=
  (nontrivial_zero_height_set_finite_v1 T).toFinset

@[simp] theorem mem_nontrivial_zero_height_finset_v1
    {T : ℝ} {rho : ℂ} :
    rho ∈ NontrivialZeroHeightFinsetV1 T ↔ rho ∈ NontrivialZeroHeightSetV1 T := by
  simp [NontrivialZeroHeightFinsetV1]

/-- Multiplicity-weighted finite Mellin sum over nontrivial zeta zeros of height at most `T`. -/
def WeilZeroHeightSumV1 (f : ℝ → ℂ) (T : ℝ) : ℂ :=
  (NontrivialZeroHeightFinsetV1 T).sum fun rho =>
    (RiemannZeroMultiplicityAtHeightV1 rho : ℂ) * mellin f rho

/-- The zero-side object in this lane is definitionally a finite `Finset.sum`. -/
theorem weil_zero_height_sum_eq_finset_v1 (f : ℝ → ℂ) (T : ℝ) :
    WeilZeroHeightSumV1 f T =
      (NontrivialZeroHeightFinsetV1 T).sum (fun rho =>
        (RiemannZeroMultiplicityAtHeightV1 rho : ℂ) * mellin f rho) := rfl

#check NontrivialZeroHeightSetV1
#check nontrivial_zero_height_set_finite_v1
#check RiemannZeroMultiplicityAtHeightV1
#check zero_height_multiplicity_pos_v1
#check NontrivialZeroHeightFinsetV1
#check WeilZeroHeightSumV1
#check weil_zero_height_sum_eq_finset_v1

#print axioms nontrivial_zero_height_set_finite_v1
#print axioms zero_height_multiplicity_pos_v1
#print axioms weil_zero_height_sum_eq_finset_v1
