...
test('V2 approval packet schema is validated before digest admission', () => {
  ...
  assert.ok(sql.includes('p_packet ?& array['))
  assert.ok(sql.includes("jsonb_typeof(p_packet -> 'packet_id') <> 'string'"))
  assert.ok(sql.includes("jsonb_typeof(p_packet -> 'created_generation') <> 'string'"))
  assert.ok(sql.includes("jsonb_typeof(e.value) <> 'string'"))
  ...
  assert.ok(sql.includes("jsonb_typeof(p_packet -> 'max_cost_minor_units') <> 'null'"))
  assert.ok(sql.includes("jsonb_typeof(p_packet -> 'currency') <> 'string'"))
