...
| CLM-231 | UNRESOLVED | Two named models differ measurably in capability on the six-task MCB-001 battery. | Proposed | D | `sovereign-omega-v2/scripts/model-capability/PREREGISTRATION.md`; graders validated in `test/unit/model-capability-graders.test.ts`; **no `results/` file exists — the battery has never been executed** (`ANTHROPIC_API_KEY` unset in the authoring sandbox) |
