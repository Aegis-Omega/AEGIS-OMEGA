...
assert.match(workflow, /\$script\s*=\s*Join-Path[\s\S]*aegis_dlss5_windows_host_bootstrap\.ps1/, 'smoke lane must bind the real bootstrap script path')
assert.match(workflow, /powershell\.exe[^\r\n]*-File\s+\$script/, 'smoke lane must invoke the bound bootstrap script through Windows PowerShell')
