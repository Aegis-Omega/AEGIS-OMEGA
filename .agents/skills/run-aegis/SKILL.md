...
```bash
PLAYWRIGHT_BROWSERS_PATH=/tmp/pw-browsers node .Codex/skills/run-aegis/driver.cjs --smoke
```
...
```bash
PLAYWRIGHT_BROWSERS_PATH=/tmp/pw-browsers node .Codex/skills/run-aegis/driver.cjs
```
...
```bash
PLAYWRIGHT_BROWSERS_PATH=/tmp/pw-browsers node .Codex/skills/run-aegis/driver.cjs --ss
```
...
Bridge endpoints: `/health`, `/telemetry`, `/node`, `/event`, `/gate_signal`, `/Codex`
