...
class FilesystemEffectAdapter:
    ...
    @staticmethod
    def _require_descriptor_open_support() -> None:
    ...
    def _acquire_root_descriptor(self) -> int:
        self._require_descriptor_open_support()
        with self._root_scope_lock:
            if self._root_fd is None:
                root_flags = os.O_RDONLY | os.O_DIRECTORY | os.O_NOFOLLOW
                root_fd: int | None = None
                try:
                    root_fd = os.open(os.fspath(self._allowed_root), root_flags)
    ...
    def _adapter_scope_commitment(self) -> str:
        with self._root_scope_lock:
            if self._root_fd is None:
                probe_fd = self._acquire_root_descriptor()
                _close_fd_safely(probe_fd)
            self._assert_root_scope_locked()
    ...
    def _observation_provenance(
        ...
        *,
        ...
        value: dict[str, Any] = {
            ...
            "filesystem_ctime_ns": observation.ctime_ns,
            "adapter_identity": self.identity,
            "adapter_version": self.version,
