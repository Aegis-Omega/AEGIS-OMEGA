...
begin
  ...
  if p_ttl_generations is null
     or p_ttl_generations < 1
     or p_ttl_generations > 16 then
  ...
  if not found then
    return query
      select 'DENIED_UNKNOWN_TASK'::text, null::text, null::bigint;
    return;
  end if;
  ...
  if v_canonical_task_digest is null
    ...
    return query
      select 'DENIED_TASK_DIGEST_MISMATCH'::text, null::text, null::bigint;
    return;
  end if;
  ...
  if exists (
    ...
    return query
      select 'DENIED_DEPENDENCY_NOT_VERIFIED'::text, null::text, null::bigint;
    return;
  end if;
    ...
    if v_lease.worker_id = p_worker_id
      ...
      return query
        select 'REPLAYED'::text,
               v_lease.lease_digest,
               v_lease.expires_generation;
    else
      return query
        select 'DENIED_ALREADY_LEASED'::text,
               v_lease.lease_digest,
               v_lease.expires_generation;
    end if;
    return;
  ...
  if v_status = 'RUNNING'
    ...
    return query
      select 'DENIED_INCONSISTENT_RUNNING'::text, null::text, null::bigint;
    return;
  end if;

  if v_status not in ('PLANNED','RUNNING') then
    return query
      select 'DENIED_TASK_STATE'::text, null::text, null::bigint;
    return;
  end if;
  ...
  return query
    select 'CLAIMED'::text, p_lease_digest, v_expires;
end;
$$;
...
begin
  ...
  if p_lease_digest is null
     or p_lease_digest !~ '^[0-9a-f]{64}$' then
    raise exception 'invalid lease_digest';
  end if;
  if p_execution_hash is null
     or p_execution_hash !~ '^[0-9a-f]{64}$' then
    raise exception 'invalid execution_hash';
  end if;
  if p_verification_hash is null
     or p_verification_hash !~ '^[0-9a-f]{64}$' then
    raise exception 'invalid verification_hash';
  end if;
  if p_receipt_root is null
     or p_receipt_root !~ '^[0-9a-f]{64}$' then
  ...
  if v_lease.worker_id <> p_worker_id
     or v_lease.lease_digest <> p_lease_digest then
