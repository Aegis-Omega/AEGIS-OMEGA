...
def validate_registry(registry: Mapping[str, Any]) -> dict[str, Any]:
    ...
    ids: set[str] = set()
    dispute_counts: dict[str, dict[str, int]] = {}
    contested_groups: set[str] = set()
    ...
    for model in registry["models"]:
        ...
        if evidence_class not in ALLOWED_EVIDENCE_CLASSES:
            raise RegistryError(
                f"{model_id}: unsupported evidence class {evidence_class!r}"
            )
        ...
        if "ESTABLISHED" in status:
            raise RegistryError(
                f"{model_id}: registry cannot self-establish disputed physics"
            )
        ...
        if signature is not None:
            ...
            if not all(type(v) is int for v in signature.values()):
                raise RegistryError(
                    f"{model_id}: V1 scaling exponents must be exact integers"
                )
        ...
        if group is not None:
            ...
            counts = dispute_counts.setdefault(
                group, {"published": 0, "critique": 0}
            )
        ...
        if status == "CONTESTED_MODEL_PREDICTION":
            if group is None:
                raise RegistryError(
                    f"{model_id}: contested model missing dispute group"
                )
    ...
    for group in contested_groups:
        ...
        if counts.get("published", 0) < 1:
            raise RegistryError(
                f"{group}: contested group missing published model"
            )
        if counts.get("critique", 0) < 2:
            raise RegistryError(
                f"{group}: contested group needs at least two counteranalyses"
            )
