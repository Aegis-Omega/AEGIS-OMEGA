...
ROOT="$(pwd)"
ASSET_DIR="$ROOT/aegisomega-webgpu/gq-full-proof-runner-v2"
WORK="/tmp/aegis-gq-full-v2"
...
payload={
  ...
  "transport_generation":"V2_CLEAN_BASE",
  "head_sha":head,
...
write_status "START" "false" "" "clean-base full replay starting"
...
run_python_suite() {
  local file="$1"
  echo "=== PYTHON $file ==="
  PYTHONPATH="$ROOT/sovereign-omega-v2/python" \
    python3 "$ROOT/sovereign-omega-v2/python/tests/$file" -v
}
...
if ! git clone --filter=blob:none --no-checkout \
  https://github.com/leanprover-community/mathlib4.git "$WORK/mathlib"; then
...
write_status "VERIFIED_EXACT_SOURCE" "true" "$LEANV" \
  "all six Python suites passed; exact F2b source compiled; axiom audit has no sorryAx"
echo "AEGIS_GQ_FULL_CLOUDFLARE_REPLAY_V2=PASS"
exit 0
