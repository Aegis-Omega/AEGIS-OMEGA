...
ROOT="$(pwd)"
ASSET_DIR="$ROOT/aegisomega-webgpu/sgm-proof-runner-v4"
WORK="/tmp/aegis-sgm"
...
SEMI_FILE="ConservativeEpistemicSemilatticeV1.lean"
AUTH_SHA256="$(sha256sum "$SRC/$AUTH_FILE" | awk '{print $1}')"
ACC_SHA256="$(sha256sum "$SRC/$ACC_FILE" | awk '{print $1}')"
SEMI_SHA256="$(sha256sum "$SRC/$SEMI_FILE" | awk '{print $1}')"
PYTHON_STATUS="NOT_RUN"

write_status() {
  python3 - "$STATUS" "$HEAD_SHA" "$AUTH_SHA256" "$ACC_SHA256" "$SEMI_SHA256" "$PYTHON_STATUS" "$1" "$2" "$3" "$4" <<'PY'
import json,sys
path,head,auth_sha,acc_sha,semi_sha,python_status,stage,ok,leanv,detail=sys.argv[1:]
payload={
  "receipt_kind":"AEGIS_SGM_CLOUDFLARE_REPLAY_V4",
  "transport_generation":"CLEAN_BASE_V4",
  "head_sha":head,
  "authority_source_sha256":auth_sha,
  "accounting_source_sha256":acc_sha,
  "semilattice_source_sha256":semi_sha,
  ...
  "python_suites":[
    ...
    "statistical_godel_machine_spec",
    "conservative_epistemic_semilattice"
  ],
  "lean_target":"4.33.1",
  "mathlib_sha":"0df444a360eaa60ab8c11dca51a86af692955474",
  "target_modules":[
...
echo "SEMILATTICE_SOURCE_SHA256=$SEMI_SHA256"
...

{
  ...
  run_python_suite test_statistical_godel_machine_spec.py &&
  run_python_suite test_conservative_epistemic_semilattice.py
} 2>&1 | tee "$PYLOG"
PY_RC=${PIPESTATUS[0]}
if [ "$PY_RC" -ne 0 ]; then
...
cp "$SRC/$SEMI_FILE" "$WORK/mathlib/$SEMI_FILE"

AUTH_COPY="$(sha256sum "$WORK/mathlib/$AUTH_FILE" | awk '{print $1}')"
ACC_COPY="$(sha256sum "$WORK/mathlib/$ACC_FILE" | awk '{print $1}')"
SEMI_COPY="$(sha256sum "$WORK/mathlib/$SEMI_FILE" | awk '{print $1}')"
echo "AUTHORITY_COPIED_SHA256=$AUTH_COPY"
echo "ACCOUNTING_COPIED_SHA256=$ACC_COPY"
echo "SEMILATTICE_COPIED_SHA256=$SEMI_COPY"
if [ "$AUTH_COPY" != "$AUTH_SHA256" ] || [ "$ACC_COPY" != "$ACC_SHA256" ] || [ "$SEMI_COPY" != "$SEMI_SHA256" ]; then
...
echo "=== COMPILE ConservativeEpistemicSemilatticeV1 ==="
if ! lean -o "$OUT/ConservativeEpistemicSemilatticeV1.olean"   "$WORK/mathlib/ConservativeEpistemicSemilatticeV1.lean"; then
  write_status "SEMILATTICE_LEAN_COMPILE_FAILED" "false" "$LEANV" "see replay.log"
  exit 0
fi
...
import ConservativeEpistemicSemilatticeV1
...
write_status "VERIFIED_EXACT_SOURCE" "true" "$LEANV"   "six Python suites passed; three exact-source Lean modules compiled; axiom audit has no sorryAx"
echo "AEGIS_SGM_CLOUDFLARE_REPLAY_V4=PASS"
exit 0
