...
ROOT="$(pwd)"
ASSET_DIR="$ROOT/aegisomega-webgpu/multi-agent-conservative-consensus-v3"
WORK="/tmp/aegis-macc"
...
CHILD_FILE="MultiAgentConservativeConsensusV1.lean"
...
payload={
  "receipt_kind":"AEGIS_MULTI_AGENT_CONSERVATIVE_CONSENSUS_REPLAY_V3",
  "transport_generation":"STABLE_MEMBER_INDUCTION_V3",
  "transport_generation":"EXPLICIT_PARENT_EXTENSIONALITY_V2",
  "head_sha":head,
...
write_status "START" "false" "" "multi-agent consensus replay starting"

echo "HEAD_SHA=$HEAD_SHA"
echo "PARENT_SOURCE_SHA256=$PARENT_SHA256"
echo "CHILD_SOURCE_SHA256=$CHILD_SHA256"

echo "=== PYTHON MULTI-AGENT CONSENSUS SUITE ==="
if ! PYTHONPATH="$ROOT/sovereign-omega-v2/python"   python3 "$ROOT/sovereign-omega-v2/python/tests/test_multi_agent_conservative_consensus.py" -v   2>&1 | tee "$PYLOG"; then
...
cat > "$WORK/mathlib/MACCAudit.lean" <<'LEAN'
...
echo "=== AXIOM AUDIT ==="
if ! lean "$WORK/mathlib/MACCAudit.lean" 2>&1 | tee "$AXLOG"; then
...
write_status "VERIFIED_EXACT_SOURCE" "true" "$LEANV"   "multi-agent Python suite passed; exact parent and child Lean modules compiled; axiom audit has no sorryAx"

echo "AEGIS_MULTI_AGENT_CONSERVATIVE_CONSENSUS_REPLAY_V3=PASS"
exit 0
