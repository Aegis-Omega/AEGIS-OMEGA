...
CAMPAIGN_EVIDENCE_VERIFICATION_RECEIPT_KIND = "CAMPAIGN_EVIDENCE_VERIFICATION_RECEIPT_V1"
...
class BenchmarkTrackSpecV1:
    ...
    def create(
        ...
        *,
        ...
        manifest_commitment = canonical_hash(
            ...
            {"unit_roots": [u.root for u in units]},
        )
        track = cls(
            track_id=track_id,
    ...
    def validate(self) -> None:
        ...
        if (
            self.published_comparability is PublishedComparability.PUBLISHED_METHODOLOGY_MATCHED
            and self.published_methodology_commitment == ZERO_HASH
        ):
        ...
        if self.measurement_resolution_bps is None:
            ...
            if (
                ...
                or not 1 <= self.measurement_resolution_bps <= 10_000
            ):
        ...
        expected_manifest = canonical_hash(
            "AEGIS_UCI8_TASK_TRIAL_MANIFEST_V1", {"unit_roots": unit_roots}
        )
        if self.task_manifest_commitment != expected_manifest:
            raise EvaluationCampaignError("TASK_MANIFEST_COMMITMENT_MISMATCH")
        if (
            not isinstance(self.repetition_count, int)
            or isinstance(self.repetition_count, bool)
        ...
        if any(indices != expected_trials for indices in trials_by_task.values()):
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "task_trial_units": [u.to_dict() for u in self.task_trial_units],
...
class EvaluationCampaignManifestV1:
        ...
        *,
        ...
        policy = canonical_hash(
            "AEGIS_UCI8_CAMPAIGN_POLICY_V1",
            {
                ...
                "baseline_selection_mode": baseline_selection_mode.value
                if isinstance(baseline_selection_mode, BaselineSelectionMode)
                else baseline_selection_mode,
    ...
    def validate(self) -> None:
        ...
        roots = [t.root for t in self.tracks]
        ...
        if self.campaign_policy_commitment != expected:
...
@dataclass(frozen=True)
class CampaignEvidenceVerificationReceiptV1:
    verification_id: str
    bundle_root: str
    campaign_root: str
    ...
    verification_status: CampaignEvidenceStatus
    key_id: str
    mac_hex: str
    receipt_kind: str = CAMPAIGN_EVIDENCE_VERIFICATION_RECEIPT_KIND

    def validate(self) -> None:
        if self.receipt_kind != CAMPAIGN_EVIDENCE_VERIFICATION_RECEIPT_KIND:
            raise EvaluationCampaignError("CAMPAIGN_VERIFICATION_RECEIPT_KIND_MISMATCH")
        _require_id("verification_id", self.verification_id)
        _require_id("key_id", self.key_id)
        for name in (
            "bundle_root",
            "campaign_root",
            ...
            _require_nonzero_hash(name, getattr(self, name))
        if not self.pair_verification_roots:
            raise EvaluationCampaignError("CAMPAIGN_VERIFICATION_PAIR_ROOTS_EMPTY")
        ...
        if len(self.pair_verification_roots) != len(set(self.pair_verification_roots)):
            raise EvaluationCampaignError("DUPLICATE_CAMPAIGN_VERIFICATION_PAIR_ROOT")
        if self.verification_status is not CampaignEvidenceStatus.COLLECTIVE_CONTRIBUTION_EVALUABLE:
            raise EvaluationCampaignError("CAMPAIGN_VERIFICATION_STATUS_INVALID")
    ...
    def root(self) -> str:
        self.validate()
        return canonical_hash("AEGIS_UCI8_CAMPAIGN_EVIDENCE_VERIFICATION_RECEIPT_V1", self.to_dict())


class PortableCheckerHMACV1:
    """Portable symmetric result/pair/campaign attestations; not public-key signatures."""
    ...
    def _campaign_mac_for_payload(self, payload: dict[str, object]) -> str:
        digest = canonical_hash("AEGIS_UCI8_CAMPAIGN_EVIDENCE_VERIFICATION_PAYLOAD_V1", payload)
        return hmac.new(self._secret_key, bytes.fromhex(digest), hashlib.sha256).hexdigest()
    ...
    def verify(
        ...
        *,
        ...
        if not hmac.compare_digest(self._mac_for_payload(attestation.unsigned_payload()), attestation.mac_hex):
    ...
    def verify_pair_verification(
        ...
        *,
        ...
        if not hmac.compare_digest(
            self._pair_mac_for_payload(attestation.unsigned_payload()), attestation.mac_hex
        ):
            raise EvaluationCampaignError("PAIR_VERIFICATION_MAC_INVALID")

    def issue_campaign_verification(
        self,
        *,
        verification_id: str,
        bundle: "CampaignEvidenceBundleV1",
    ) -> CampaignEvidenceVerificationReceiptV1:
        _require_id("verification_id", verification_id)
        bundle.validate()
        if not _is_campaign_bundle_issued(bundle):
            raise EvaluationCampaignError("CAMPAIGN_VERIFICATION_ISSUER_REQUIRES_ISSUED_BUNDLE")
        if not bundle.pair_verification_roots or len(bundle.pair_verification_roots) != len(bundle.paired_trial_roots):
            raise EvaluationCampaignError("CAMPAIGN_VERIFICATION_REQUIRES_PORTABLE_PAIR_VERIFICATIONS")
        if bundle.evidence_status is not CampaignEvidenceStatus.HELD_OUT_EVIDENCE_COMPLETE:
            raise EvaluationCampaignError("CAMPAIGN_VERIFICATION_REQUIRES_HELD_OUT_EVIDENCE")
        unsigned = {
            "receipt_kind": CAMPAIGN_EVIDENCE_VERIFICATION_RECEIPT_KIND,
            "verification_id": verification_id,
            "bundle_root": bundle.root,
            "campaign_root": bundle.campaign_root,
            "pair_verification_roots": list(bundle.pair_verification_roots),
            "benchmark_adapter_executable_commitment": bundle.benchmark_adapter_executable_commitment,
            "runner_environment_commitment": bundle.runner_environment_commitment,
            "execution_receipt_bundle_commitment": bundle.execution_receipt_bundle_commitment,
            "verification_status": CampaignEvidenceStatus.COLLECTIVE_CONTRIBUTION_EVALUABLE.value,
            "key_id": self._key_id,
        }
        receipt = CampaignEvidenceVerificationReceiptV1(
            verification_id=verification_id,
            bundle_root=bundle.root,
            campaign_root=bundle.campaign_root,
            pair_verification_roots=bundle.pair_verification_roots,
            benchmark_adapter_executable_commitment=bundle.benchmark_adapter_executable_commitment,
            runner_environment_commitment=bundle.runner_environment_commitment,
            execution_receipt_bundle_commitment=bundle.execution_receipt_bundle_commitment,
            verification_status=CampaignEvidenceStatus.COLLECTIVE_CONTRIBUTION_EVALUABLE,
            key_id=self._key_id,
            mac_hex=self._campaign_mac_for_payload(unsigned),
        )
        receipt.validate()
        return receipt

    def verify_campaign_verification(
        self,
        *,
        bundle: "CampaignEvidenceBundleV1",
        receipt: CampaignEvidenceVerificationReceiptV1,
        ...
        bundle.validate()
        receipt.validate()
        if receipt.bundle_root != bundle.root:
            raise EvaluationCampaignError("CAMPAIGN_VERIFICATION_BUNDLE_ROOT_MISMATCH")
        if receipt.campaign_root != bundle.campaign_root:
            raise EvaluationCampaignError("CAMPAIGN_VERIFICATION_CAMPAIGN_ROOT_MISMATCH")
        if receipt.pair_verification_roots != bundle.pair_verification_roots:
            raise EvaluationCampaignError("CAMPAIGN_VERIFICATION_PAIR_ROOTS_MISMATCH")
        if receipt.benchmark_adapter_executable_commitment != bundle.benchmark_adapter_executable_commitment:
            raise EvaluationCampaignError("CAMPAIGN_VERIFICATION_ADAPTER_COMMITMENT_MISMATCH")
        if receipt.runner_environment_commitment != bundle.runner_environment_commitment:
            raise EvaluationCampaignError("CAMPAIGN_VERIFICATION_RUNNER_ENVIRONMENT_MISMATCH")
        if receipt.execution_receipt_bundle_commitment != bundle.execution_receipt_bundle_commitment:
            raise EvaluationCampaignError("CAMPAIGN_VERIFICATION_EXECUTION_BUNDLE_MISMATCH")
        if receipt.key_id != self._key_id:
            raise EvaluationCampaignError("CAMPAIGN_VERIFICATION_KEY_ID_MISMATCH")
        if not hmac.compare_digest(
            self._campaign_mac_for_payload(receipt.unsigned_payload()), receipt.mac_hex
        ):
            raise EvaluationCampaignError("CAMPAIGN_VERIFICATION_MAC_INVALID")
...
class PairedBenchmarkTrialV1:
    ...
    def create(
        ...
        *,
        ...
        if (
            system_result.task_spec_root != baseline_result.task_spec_root
            or system_result.trial_index != baseline_result.trial_index
        ):
        ...
        if unit.root not in {u.root for u in track.task_trial_units}:
        ...
        if (
            system_result.budget_commitment != track.budget_commitment
            or baseline_result.budget_commitment != track.budget_commitment
        ):
            raise EvaluationCampaignError("BUDGET_COMMITMENT_MISMATCH")
        if (
            system_result.checker_commitment != track.scorer_commitment
            or baseline_result.checker_commitment != track.scorer_commitment
        ):
            raise EvaluationCampaignError("SCORER_COMMITMENT_MISMATCH")
        if track.root not in {t.root for t in campaign.tracks}:
            raise EvaluationCampaignError("TRACK_NOT_IN_CAMPAIGN")
        args = (expected_run_id, system_attestation, baseline_attestation, attestation_verifier)
        requested = any(value is not None for value in args)
...
_ISSUED_CAMPAIGN_BUNDLES_LOCK = threading.RLock()
_ISSUED_CAMPAIGN_BUNDLES: dict[int, weakref.ReferenceType["CampaignEvidenceBundleV1"]] = {}


def _register_campaign_bundle(bundle: "CampaignEvidenceBundleV1") -> "CampaignEvidenceBundleV1":
    key = id(bundle)

    def cleanup(ref: weakref.ReferenceType["CampaignEvidenceBundleV1"], *, object_id: int = key) -> None:
        with _ISSUED_CAMPAIGN_BUNDLES_LOCK:
            if _ISSUED_CAMPAIGN_BUNDLES.get(object_id) is ref:
                _ISSUED_CAMPAIGN_BUNDLES.pop(object_id, None)

    ref = weakref.ref(bundle, cleanup)
    with _ISSUED_CAMPAIGN_BUNDLES_LOCK:
        _ISSUED_CAMPAIGN_BUNDLES[key] = ref
    return bundle


def _is_campaign_bundle_issued(bundle: "CampaignEvidenceBundleV1") -> bool:
    with _ISSUED_CAMPAIGN_BUNDLES_LOCK:
        ref = _ISSUED_CAMPAIGN_BUNDLES.get(id(bundle))
        return ref is not None and ref() is bundle
...
class CampaignEvidenceBundleV1:
    ...
    def create(
        ...
        *,
        ...
        if portable and (
            pair_verification_verifier is None or len(verification_tuple) != len(pair_tuple)
        ):
            raise EvaluationCampaignError("PAIR_VERIFICATION_ARGUMENTS_INCOMPLETE")
        if len({verification.root for verification in verification_tuple}) != len(verification_tuple):
        ...
        track_roots = {track.root for track in campaign.tracks}
        ...
        for index, pair in enumerate(pair_tuple):
            ...
            if pair.track_root not in track_roots:
        ...
        bundle.validate()
        return _register_campaign_bundle(bundle)
    ...
    def validate(self) -> None:
        ...
        if self.evidence_status is CampaignEvidenceStatus.COLLECTIVE_CONTRIBUTION_EVALUABLE:
            raise EvaluationCampaignError("COLLECTIVE_STATUS_REQUIRES_VERIFICATION_RECEIPT")
        for name in (
            "benchmark_adapter_executable_commitment",
            "runner_environment_commitment",
            "execution_receipt_bundle_commitment",
